...
struct PJLINK_API FPJLinkDiagnosticData
{
    ...
    // 다이제스트 모드 보고서 생성 (모든 항목 대신 요약만 생성)
    FString GenerateDigestReport(int32 MaxReportEntries = 20) const
    {
        FString Report = FString::Printf(TEXT("PJLink Diagnostic Digest Report\n"));
        Report += FString::Printf(TEXT("ID: %s\n"), *DiagnosticId.ToString());
        Report += FString::Printf(TEXT("Start Time: %s\n"), *StartTime.ToString());
        Report += FString::Printf(TEXT("Last Update: %s\n"), *LastUpdateTime.ToString());
        Report += FString::Printf(TEXT("Total Entry Count: %d (showing %d)\n\n"),
            Entries.Num(), FMath::Min(MaxReportEntries, Entries.Num()));

        // 가장 최근 N개의 항목만 표시
        int32 StartIdx = FMath::Max(0, Entries.Num() - MaxReportEntries);
